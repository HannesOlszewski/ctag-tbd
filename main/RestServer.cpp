...
static esp_err_t send_file(const char *filepath, httpd_req_t *req,
...
esp_err_t RestServer::get_plugins_get_handler(httpd_req_t *req) {
  ESP_LOGD(
      ...
      "1: Mem freesize internal %d, largest block %d, free SPIRAM %d, largest "
      "block SPIRAM %d!",
...
esp_err_t RestServer::get_params_ui_plugin_get_handler(httpd_req_t *req) {
  ESP_LOGD(
  ...
  httpd_resp_set_type(req, "application/json");
  auto plugin = CTAG::AUDIO::SoundProcessorManager::GetStringID(ch);
  ESP_LOGD(REST_TAG, "Get plugin params ui for channel %d and plugin %s", ch,
           plugin.c_str());

  char filepath[FILE_PATH_MAX];
  auto *rest_context = static_cast<rest_server_context_t *>(req->user_ctx);
  strlcpy(filepath,
          (string(CTAG::RESOURCES::spiffsRoot + "/data/sp/mui-") + plugin +
           string(".jsn"))
              .c_str(),
          sizeof(filepath));
...
esp_err_t RestServer::get_params_p_plugin_get_handler(httpd_req_t *req) {
  ESP_LOGD(
      "get_params_plugin_get_handler",
      "1: Mem freesize internal %d, largest block %d, free SPIRAM %d, largest "
      "block SPIRAM %d!",
  ...
  auto *rest_context = static_cast<rest_server_context_t *>(req->user_ctx);
  strlcpy(filepath,
          (string(CTAG::RESOURCES::spiffsRoot + "/data/sp/mp-") + plugin +
           string(".jsn"))
              .c_str(),
          sizeof(filepath));
...
esp_err_t RestServer::set_plugin_param_get_handler(httpd_req_t *req) {
  ...
  std::string sid(id);
  ch -= 0x30;
  ESP_LOGD(REST_TAG, "Setting chan %d param %s key %s value %d", ch, id,
           key.c_str(), val);
  if (ch == 0 || ch == 1)
...
esp_err_t RestServer::get_presets_get_handler(httpd_req_t *req) {
  ...
  if (ch == 1 && !CTAG::AUDIO::SoundProcessorManager::IsStereo(0)) {
    const char *res =
        CTAG::AUDIO::SoundProcessorManager::GetCStrJSONGetPresets(ch);
    if (nullptr != res)
      httpd_resp_sendstr(req, res);
...
esp_err_t RestServer::save_preset_get_handler(httpd_req_t *req) {
  ...
  if (ch == 0 || ch == 1)
    CTAG::AUDIO::SoundProcessorManager::ChannelSavePreset(ch, string(name),
                                                          atoi(number));
  httpd_resp_set_type(req, "text/html");
  httpd_resp_send(req, NULL, 0);
  return ESP_OK;
...
static void ws_async_send(void *arg) {
  ...
  auto *resp_arg = static_cast<struct async_resp_arg *>(arg);
...
static esp_err_t trigger_async_send(httpd_handle_t handle, httpd_req_t *req) {
  auto *resp_arg = static_cast<struct async_resp_arg *>(
      heap_caps_malloc(sizeof(struct async_resp_arg), MALLOC_CAP_8BIT));
  if (resp_arg == NULL) {
...
static esp_err_t ws_handler(httpd_req_t *req) {
  ...
  if (ret != ESP_OK) {
        ESP_LOGE(REST_TAG, "httpd_ws_recv_frame failed to get frame len with %d", ret); return ret;
  }
  ESP_LOGI(REST_TAG, "frame len is %d", ws_pkt.len);
  if (ws_pkt.len) {
    /* ws_pkt.len + 1 is for NULL termination as we are expecting a
    string */
...
static esp_err_t ping_get_handler(httpd_req_t *req) {
  ...
  httpd_req_get_url_query_str(req, s, 128);
  httpd_query_key_value(s, "q", v, 128);
  std::string id(v);

  httpd_resp_sendstr(req, v);
  return ESP_OK;
}
...
esp_err_t RestServer::StartRestServer() {
  ...
  */
  ...
  httpd_uri_t get_plugins_get_uri = {.uri = "/api/v1/getPlugins",
                                     .method = HTTP_GET,
                                     .handler =
                                         &RestServer::get_plugins_get_handler,
  ...
  httpd_uri_t get_params_ui_plugin_get_uri = {
      ...
      .method = HTTP_GET,
      .handler = &RestServer::get_params_ui_plugin_get_handler,
      .user_ctx = rest_context};
  ...
  httpd_uri_t get_params_p_plugin_get_uri = {
      ...
      .method = HTTP_GET,
      .handler = &RestServer::get_params_p_plugin_get_handler,
      .user_ctx = rest_context};
  ...
  /* get all presets */
  httpd_uri_t get_presets_get_uri = {.uri = "/api/v1/getPresets*",
                                     .method = HTTP_GET,
                                     .handler =
                                         &RestServer::get_presets_get_handler,
                                     .user_ctx = rest_context};
  ...
  /* save a preset */
  httpd_uri_t save_preset_get_uri = {.uri = "/api/v1/savePreset*",
                                     .method = HTTP_GET,
                                     .handler =
                                         &RestServer::save_preset_get_handler,
                                     .user_ctx = rest_context};
  httpd_register_uri_handler(server, &save_preset_get_uri);

  /* load a preset */
  httpd_uri_t load_preset_get_uri = {.uri = "/api/v1/loadPreset*",
                                     .method = HTTP_GET,
                                     .handler =
                                         &RestServer::load_preset_get_handler,
                                     .user_ctx = rest_context};
  ...
  httpd_uri_t reboot_handler_get_uri = {.uri = "/api/v1/reboot",
                                        .method = HTTP_GET,
  ...
  /* get io caps */
  httpd_uri_t io_caps_handler_get_uri = {.uri = "/api/v1/getIOCaps",
                                         .method = HTTP_GET,
                                         .handler =
                                             &RestServer::get_iocaps_handler,
                                         .user_ctx = rest_context};
  ...
  /* set spiffs upload */
  httpd_uri_t ota_post_uri = {.uri = "/api/v1/otaAPI*",
                              .method = HTTP_POST,
  ...
  /* favorite handler*/
  httpd_uri_t favorite_get_uri = {.uri = "/api/v1/favorite*",
                                  .method = HTTP_POST,
  ...
  /* set sample upload */
  httpd_uri_t srom_post_uri = {.uri = "/api/v1/srom*",
                               .method = HTTP_POST,
  ...
  /* websocket handler */
  httpd_uri_t ws_uri = {.uri = "/api/v1/ws",
                        .method = HTTP_GET,
                        .handler = &ws_handler,
                        .user_ctx = rest_context,
                        .is_websocket = true};
  ...
  httpd_uri_t ping_get_uri = {.uri = "/api/v1/ping",
                              .method = HTTP_GET,
                              .handler = &ping_get_handler,
                              .user_ctx = rest_context};
  httpd_register_uri_handler(server, &ping_get_uri);
...
esp_err_t RestServer::set_configuration_post_handler(httpd_req_t *req) {
  ...
  char *content =
      (char *)heap_caps_malloc(req->content_len + 1, MALLOC_CAP_SPIRAM);
...
esp_err_t RestServer::get_preset_json_handler(httpd_req_t *req) {
  ...
  if (pLastSlash) {
    ...
    ESP_LOGD(REST_TAG, "Sending all preset data of plugin %s as JSON",
             pluginID);
    const char *json =
...
esp_err_t RestServer::reboot_handler(httpd_req_t *req) {
  ...
  if (doCal)
    CTAG::CAL::Calibration::RequestCalibrationOnReboot();
  httpd_resp_set_type(req, "text/html");
  httpd_resp_send(req, NULL, 0);
  esp_restart();
  return ESP_OK;
...
esp_err_t RestServer::ota_handler(httpd_req_t *req) {
  ...
  ESP_LOGI("HTTPD", "OTA request type %d, last request was %d, expecting %d",
           otaRequest, lastOtaRequest, lastOtaRequest + 1);
...
esp_err_t RestServer::set_calibration_post_handler(httpd_req_t *req) {
  ESP_LOGD(
      ...
      heap_caps_get_free_size(MALLOC_CAP_SPIRAM),
      heap_caps_get_largest_free_block(MALLOC_CAP_SPIRAM));
  ESP_LOGI("REST", "Set calibration post handler: content length %d",
           req->content_len);
  char *content =
      (char *)heap_caps_malloc(req->content_len + 1, MALLOC_CAP_SPIRAM);
  int ret = httpd_req_recv(req, content, req->content_len);
...
esp_err_t RestServer::set_preset_json_handler(httpd_req_t *req) {
  ...
  if (pLastSlash) {
    strcpy(pluginID, pLastSlash + 1);
    char *content =
        (char *)heap_caps_calloc(1, req->content_len + 1, MALLOC_CAP_SPIRAM);
    ESP_LOGD(REST_TAG, "Storing data for %s as JSON, content length %d",
             pluginID, req->content_len);
...
esp_err_t RestServer::srom_handler(httpd_req_t *req) {
  ...
  if (cmd.compare("erase") == 0) {
    ...
    ESP_LOGI("REST", "Erasing flash start %d, size %d!",
             CONFIG_SAMPLE_ROM_START_ADDRESS, CONFIG_SAMPLE_ROM_SIZE);
    // ESP_ERROR_CHECK(spi_flash_erase_range(CONFIG_SAMPLE_ROM_START_ADDRESS,
    // CONFIG_SAMPLE_ROM_SIZE));
    ESP_ERROR_CHECK(esp_flash_erase_region(
        NULL, CONFIG_SAMPLE_ROM_START_ADDRESS, CONFIG_SAMPLE_ROM_SIZE));
    httpd_resp_set_type(req, "text/html");
    httpd_resp_send(req, NULL, 0);
  ...
  if (cmd.compare("upRaw") == 0) {
    ...
    char *buffer =
        (char *)heap_caps_malloc(4096, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (buffer == NULL) {
      httpd_resp_send_500(req);
      return ESP_ERR_NO_MEM;
    }
    ...
    while (remaining > 0) {
      ...
      if (data_read < 0) {
        ...
        // spi_flash_write(CONFIG_SAMPLE_ROM_START_ADDRESS + offset, buffer,
        // data_read);
        esp_flash_write(NULL, buffer, CONFIG_SAMPLE_ROM_START_ADDRESS + offset,
                        data_read);
      }
      offset += data_read;
      remaining -= data_read;
